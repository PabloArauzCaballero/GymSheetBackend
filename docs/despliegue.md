...
2. Generar una deploy key SSH (par nuevo — GitHub no admite la misma llave en dos repos),
   ...
   `GymSheetBackend`.
3. Crear el recurso Postgres en el mismo proyecto de Coolify y copiar su alias interno a
   `DB_HOST` (puerto 5432, sin `-pooler` si en algún momento se usa Neon en vez de Postgres de
   Coolify).
4. Crear el recurso Redis si se quiere rate-limit compartido entre instancias (`REDIS_URL`); si se
   deja vacío, el backend degrada a límites por proceso — aceptable con una sola réplica.
5. Pegar en la interfaz de la aplicación (no en el repo) todas las variables de `.env.example`
   con sus valores de producción: como mínimo `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`,
   `DB_PASSWORD`, `CORS_ORIGINS`, `WHATSAPP_MEMBERSHIP_PHONE`, `SEED_ADMIN_EMAIL`,
   `SEED_ADMIN_PASSWORD`, `MEDIA_STORAGE_PUBLIC_BASE_URL`. `JWT`/`SESSION` secrets si existen en el
   código de auth deben generarse nuevos, nunca reusar los de otro entorno.
