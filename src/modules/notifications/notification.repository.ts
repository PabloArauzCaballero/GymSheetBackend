...
export class NotificationRepository {
  constructor(
    @InjectModel(NotificationModel) private readonly messages: typeof NotificationModel,
    @InjectModel(NotificationPreferenceModel) private readonly preferences: typeof NotificationPreferenceModel,
    @InjectModel(DeliveryAttemptModel) private readonly attempts: typeof DeliveryAttemptModel,
    private readonly sequelize: Sequelize,
  ) {}

  listForUser(userId: string, filters: NotificationListInput) {
    return this.messages.findAndCountAll({
      where: { recipientUserId: userId, ...(filters.estado ? { status: filters.estado } : {}) },
  ...
  findForUser(id: string, userId: string) {
    return this.messages.findOne({ where: { id, recipientUserId: userId } });
  }

  async markRead(message: NotificationModel) {
    await message.update({ status: NotificationStatus.READ, readAt: message.readAt ?? new Date() });
    return message;
  }

  findPreference(userId: string) { return this.preferences.findOne({ where: { userId } }); }
  async upsertPreference(userId: string, input: UpdateNotificationPreferenceInput) {
    const existing = await this.findPreference(userId);
    if (existing) { await existing.update(input); return existing; }
  ...
  findExpiringMemberships(today: string, limit: number) {
    return this.sequelize.query<ExpiringMembershipCandidate>(
      `SELECT m.id AS "membershipId", m.user_id AS "userId", p.name AS "planName",
              m.ends_on::text AS "endsOn",
              (m.ends_on - CAST(:today AS date))::integer AS "daysRemaining",
              COALESCE(pref.preferred_channel, 'IN_APP') AS channel
       ...
       WHERE m.status = 'ACTIVE' AND p.status = 'ACTIVE'
         AND m.starts_on <= CAST(:today AS date) AND m.ends_on >= CAST(:today AS date)
         AND COALESCE(pref.membership_expiry_enabled, true) = true
         AND EXISTS (
           SELECT 1 FROM jsonb_array_elements_text(p.reminder_days) AS reminder(day)
           WHERE reminder.day::integer = (m.ends_on - CAST(:today AS date))
         )
       ORDER BY m.ends_on ASC, m.id ASC LIMIT :limit`,
  ...
  createDeliveryAttempt(input: Record<string, unknown>, transaction: Transaction) {
